...
//!
//! Первая структура ACPI, указывающая на RSDT/XSDT.
...
impl Rsdp {
    ...
    pub fn validate(&self) -> Result<(), AcpiError> {
        ...
        // Check the signature - use read_unaligned for packed struct field access
        // RSDP may not be 16-byte aligned on UEFI systems
        let signature = unsafe { core::ptr::read_unaligned(addr_of!(self.signature)) };
        if signature != RSDP_SIGNATURE {
            return Err(AcpiError::RsdpIncorrectSignature);
        }

        // Check the OEM id is valid UTF-8 - also use read_unaligned
        let oem_id = unsafe { core::ptr::read_unaligned(addr_of!(self.oem_id)) };
        if str::from_utf8(&oem_id).is_err() {
         ...
         */
        let revision = unsafe { core::ptr::read_unaligned(addr_of!(self.revision)) };
        let length = if revision > 0 {
            // For Version 2.0+, include the number of bytes specified by `length`
            let len = unsafe { core::ptr::read_unaligned(addr_of!(self.length)) };
            len as usize
        } else {
            RSDP_V1_LENGTH
        };
    ...
    pub fn signature(&self) -> [u8; 8] {
        use core::ptr::addr_of;
        unsafe { core::ptr::read_unaligned(addr_of!(self.signature)) }
    }

    pub fn checksum(&self) -> u8 {
        use core::ptr::addr_of;
        unsafe { core::ptr::read_unaligned(addr_of!(self.checksum)) }
    }

    pub fn oem_id(&self) -> Result<&str, AcpiError> {
        use core::ptr::addr_of;
        let oem_id = unsafe { core::ptr::read_unaligned(addr_of!(self.oem_id)) };
        // Return static str or error - we can't return reference to local
        // For now just validate
        if str::from_utf8(&oem_id).is_err() {
            return Err(AcpiError::RsdpInvalidOemId);
        }
        // This is safe because we validated above and the data doesn't move
        unsafe { str::from_utf8_unchecked(&self.oem_id) }
            .try_into()
            .map_err(|_| AcpiError::RsdpInvalidOemId)
    }

    pub fn revision(&self) -> u8 {
        use core::ptr::addr_of;
        unsafe { core::ptr::read_unaligned(addr_of!(self.revision)) }
    }

    pub fn rsdt_address(&self) -> u32 {
        use core::ptr::addr_of;
        unsafe { core::ptr::read_unaligned(addr_of!(self.rsdt_address)) }
    }

    pub fn length(&self) -> u32 {
        use core::ptr::addr_of;
        let rev = unsafe { core::ptr::read_unaligned(addr_of!(self.revision)) };
        assert!(
            rev > 0,
            "Tried to read extended RSDP field with ACPI Version 1.0"
        );
        unsafe { core::ptr::read_unaligned(addr_of!(self.length)) }
    }
    ...
    pub fn xsdt_address(&self) -> u64 {
        ...
        unsafe { core::ptr::read_unaligned(addr_of!(self.xsdt_address)) }
    }
    ...
    pub fn ext_checksum(&self) -> u8 {
        ...
        unsafe { core::ptr::read_unaligned(addr_of!(self.ext_checksum)) }
    }
}
