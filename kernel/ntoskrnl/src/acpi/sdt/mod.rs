...
//!
//! Модули для различных ACPI таблиц.
...
impl SdtHeader {
    ...
    pub unsafe fn validate(&self, signature: Signature) -> Result<(), AcpiError> {
        unsafe {
            ...
            // Check the signature - use read_unaligned for packed struct
            let self_sig = core::ptr::read_unaligned(addr_of!(self.signature));
            if self_sig != signature || str::from_utf8(&self_sig.0).is_err() {
                return Err(AcpiError::SdtInvalidSignature(signature));
            }

            // Check the OEM id
            let oem_id = core::ptr::read_unaligned(addr_of!(self.oem_id));
            if str::from_utf8(&oem_id).is_err() {
                return Err(AcpiError::SdtInvalidOemId(signature));
            }

            // Check the OEM table id
            let oem_table_id = core::ptr::read_unaligned(addr_of!(self.oem_table_id));
            if str::from_utf8(&oem_table_id).is_err() {
                return Err(AcpiError::SdtInvalidTableId(signature));
            }

            // Check the checksum
            let length = core::ptr::read_unaligned(addr_of!(self.length));
            let table_bytes = core::slice::from_raw_parts(
    ...
    pub fn oem_id(&self) -> Result<&str, AcpiError> {
        use core::ptr::addr_of;
        let sig = unsafe { core::ptr::read_unaligned(addr_of!(self.signature)) };
        let oem_id = unsafe { core::ptr::read_unaligned(addr_of!(self.oem_id)) };
        if str::from_utf8(&oem_id).is_err() {
            return Err(AcpiError::SdtInvalidOemId(sig));
        }
        // Safe: validated above, data doesn't move
        Ok(unsafe { str::from_utf8_unchecked(&self.oem_id) })
    }

    pub fn oem_table_id(&self) -> Result<&str, AcpiError> {
        use core::ptr::addr_of;
        let sig = unsafe { core::ptr::read_unaligned(addr_of!(self.signature)) };
        let oem_table_id = unsafe { core::ptr::read_unaligned(addr_of!(self.oem_table_id)) };
        if str::from_utf8(&oem_table_id).is_err() {
            return Err(AcpiError::SdtInvalidTableId(sig));
        }
        Ok(unsafe { str::from_utf8_unchecked(&self.oem_table_id) })
    }

    #[inline]
    pub fn oem_revision(&self) -> u32 {
        use core::ptr::addr_of;
        unsafe { core::ptr::read_unaligned(addr_of!(self.oem_revision)) }
    }
