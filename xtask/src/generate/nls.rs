//! Генерация unicode.nls
//!
//! Скачивает Unicode Character Database (UCD) и генерирует NLS файл в формате XNLS v1
...
const UCD_BASE_URL: &str = "https://www.unicode.org/Public/UCD/latest/ucd";
const UCD_FILES: &[&str] = &[
    "UnicodeData.txt",
    "SpecialCasing.txt",
...
/// Генерирует unicode.nls
pub fn generate() -> Result<()> {
    log::info!("=== Generating unicode.nls ===");

    let paths = Paths::new()?;

    let ucd_dir = paths.build_dir.join("ucd/latest/ucd");
    let output_dir = paths.sysroot_dir.join("XenOS/system32");
    let output_path = output_dir.join("unicode.nls");

    // Скачиваем UCD файлы если нужно
    ensure_ucd_files(&ucd_dir)?;
    ...
    let generated_at_unix = std::time::SystemTime::now()
        ...
        .unwrap_or_default()
        .as_secs();
    let source_dir = ucd_dir.to_string_lossy();
    let meta = format!(
        "generator=xtask-nslgen\nsource_dir={source_dir}\ngenerated_at_unix={generated_at_unix}\n"
    );

    let ucd_tag: [u8; 4] = *b"UCD ";
    let ucd_revision: u32 = 0;

    let bytes = build_xnls_v1(&meta, &upper, &lower, &props, ucd_tag, ucd_revision)?;
...
/// Скачивает UCD файлы если отсутствуют
fn ensure_ucd_files(ucd_dir: &std::path::Path) -> Result<()> {
    ensure_dir(ucd_dir)?;

    for file in UCD_FILES {
        let path = ucd_dir.join(file);
        if !path.exists() {
            let url = format!("{}/{}", UCD_BASE_URL, file);
...
fn apply_unicode_data(
    ...
    for (lineno, raw) in text.lines().enumerate() {
        ...
        if fields.len() < 15 {
            anyhow::bail!("UnicodeData.txt:{}: too few fields", lineno + 1);
        }

        let cp = parse_hex_u32(fields[0])
            .with_context(|| format!("UnicodeData.txt:{}", lineno + 1))?;
        ...
        if name.ends_with(", Last>") {
            if let Some((start, gc0, _prefix)) = pending_range.take() {
                if start > cp {
                    anyhow::bail!("UnicodeData.txt:{}: invalid range", lineno + 1);
                }
                for v in start..=cp {
                    if v <= 0xFFFF {
...
fn apply_special_casing(text: &str, upper: &mut [u16], lower: &mut [u16]) -> Result<()> {
    // SpecialCasing.txt:
    ...
    for (lineno, raw) in text.lines().enumerate() {
        ...
        if fields.len() < 5 {
            anyhow::bail!("SpecialCasing.txt:{}: too few fields", lineno + 1);
        }
        let cp = parse_hex_u32(fields[0])
            .with_context(|| format!("SpecialCasing.txt:{}", lineno + 1))?;
        if cp > 0xFFFF {
            continue;
        }
        let cond = fields[4];
...
fn apply_derived_core_properties(text: &str, props: &mut [u16]) -> Result<()> {
    ...
    for (lineno, raw) in text.lines().enumerate() {
        ...
        let (range, prop) = split_prop_line(line)
            .ok_or_else(|| anyhow::anyhow!("DerivedCoreProperties.txt:{}: bad line", lineno + 1))?;
        let (start, end) = parse_range(range)
            .with_context(|| format!("DerivedCoreProperties.txt:{}", lineno + 1))?;
...
fn apply_prop_list(text: &str, props: &mut [u16]) -> Result<()> {
    ...
    for (lineno, raw) in text.lines().enumerate() {
        ...
        let (range, prop) = split_prop_line(line)
            .ok_or_else(|| anyhow::anyhow!("PropList.txt:{}: bad line", lineno + 1))?;
        if prop != "White_Space" {
            continue;
        }
        let (start, end) = parse_range(range)
            .with_context(|| format!("PropList.txt:{}", lineno + 1))?;
        let s = start.min(0xFFFF);
        let e = end.min(0xFFFF);
        for cp in s..=e {
...
fn build_xnls_v1(
    ...
    if upper.len() != BMP_LEN || lower.len() != BMP_LEN || props.len() != BMP_LEN {
        anyhow::bail!("invalid table sizes");
    }
    ...
    for ch in &mut chunks {
        ...
        cur = cur.checked_add(align8(ch.length)).ok_or_else(|| anyhow::anyhow!("file too large"))?;
    }
    ...
    let file_size_usize: usize = file_size
        .try_into()
        .map_err(|_| anyhow::anyhow!("file too large for this host"))?;
    ...
    // TOC.
    let toc_len_usize: usize = toc_len
        .try_into()
        .map_err(|_| anyhow::anyhow!("toc too large for this host"))?;
    ...
    for ch in &chunks {
        let want: usize = ch
            ...
            .map_err(|_| anyhow::anyhow!("file too large for this host"))?;
        if out.len() > want {
            anyhow::bail!("internal layout error: overlap");
        }
        while out.len() < want {
            out.push(0);
        }
    ...
    if out.len() != file_size_usize {
        anyhow::bail!("internal size mismatch");
    }
    write_header_xnls(
        &mut out,
        file_size,
